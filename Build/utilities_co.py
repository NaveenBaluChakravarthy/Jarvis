...
class DataUtilities:
    ...
    def tokenize(self):
        ...
        self.tokenizer = tfds.features.text.SubwordTextEncoder.build_from_corpus(
            self.questions + self.answers, target_vocab_size=C_max_vocab_size)
        C_start_token = [self.tokenizer.vocab_size]
        C_stop_token = [self.tokenizer.vocab_size + 1]
        C_actual_vocab_size = self.tokenizer.vocab_size + 2

        for (q, a) in zip(self.questions, self.answers):
            self.qns = C_start_token + self.tokenizer.encode(q) + C_stop_token
            self.ans = C_start_token + self.tokenizer.encode(a) + C_stop_token
            if len(self.qns) <= C_max_length and len(self.ans) <= C_max_length:
        ...
        self.tokenized_questions = tf.keras.preprocessing.sequence.pad_sequences(
            self.tokenized_questions, maxlen=C_max_length, padding='post')
        self.tokenized_answers = tf.keras.preprocessing.sequence.pad_sequences(
            self.tokenized_answers, maxlen=C_max_length, padding='post')
...
class PositionalEncoding(tf.keras.layers.Layer):
    ...
    def positional_encoding(self):
        ...
        angle_rads = self.get_angles(tf.range(C_actual_vocab_size, dtype=tf.float32)[:, tf.newaxis],
                                     tf.range(C_d_model, dtype=tf.float32)[tf.newaxis, :])
